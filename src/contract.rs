...
pub fn query(
    ...
    match msg {
        QueryMsg::ValidateWallet { address } => {
            validate_receiver_wallet(deps, address)
        },
...
pub fn redeem_gift_card(
    ...
    if env.block.time.seconds() > gift_card.expiry() {
        return Err(ContractError::Std(StdError::generic_err("Gift card expired")));
    }
    ...
    if computed_hash != gift_card.hash() {
        return Err(ContractError::Std(StdError::generic_err("Invalid gift card")));
    }
    ...
    REDEEMED_GIFTS.save(deps.storage, (recipient.clone(), gift_id.to_string()), &redeemed_gift)?;
...
pub fn validate_receiver_wallet(deps: Deps, wallet_address: Addr) -> StdResult<Binary> {
    ...
    let response = WalletVerification::new(
        "Verified Wallet Address".to_string(),
    );
...
mod tests {
    ...
    use cosmwasm_std::{coins, from_json, Coin, Timestamp, Uint128};
    ...
    fn full_gift_card_flow() {
        ...
        let gift_exists = UNCLAIMED_GIFTS.may_load(deps.as_ref().storage, gift_id.clone()).unwrap();
        assert!(gift_exists.is_none());
        
        // 9. Verify the gift appears in redeemed gifts
        let recipient_addr = Addr::unchecked(RECIPIENT);
        let redeemed = REDEEMED_GIFTS.may_load(deps.as_ref().storage, (recipient_addr, gift_id)).unwrap();
...
fn test_expired_gift() {
    ...
    match err {
        ContractError::Std(err) => {
            assert!(err.to_string().contains("Gift card expired"), 
                   "Expected 'Gift card expired' but got: {}", err.to_string());
        },
        _ => panic!("Expected StdError with 'Gift card expired' but got: {:?}", err),
    }
}
    #[test]
    fn test_wallet_queries() {
        ...
        let query_msg = QueryMsg::GetWalletByEmail { email: EMAIL.to_string() };
        ...
        // Validate wallet
        let query_msg = QueryMsg::ValidateWallet { address: Addr::unchecked(SENDER) };
        let bin = query(deps.as_ref(), env.clone(), query_msg).unwrap();
